...
/// <summary>
/// The org-level RBAC permission vocabulary (resource:action), mined from the Pulumi console which gates UI
/// on these strings (157 permissions across 54 resources). The console fetches the caller's granted set and
/// does <c>new Set(resp)</c>. Admins are granted the full set; finer per-role grants are the ADR-0007 follow-up.
/// </summary>
public static class RbacPermissions
{
    ...
    /// <summary>
    /// The permissions granted to a member of <paramref name="role"/>. Admins get everything; other members
    /// get the read/list subset (the ADR-0007 follow-up replaces this with persisted per-role grants).
    /// </summary>
    public static IReadOnlyList<string> ForRole(string role)
        => role == "admin"
            ? All
            : All.Where(p => p.EndsWith(":read") || p.EndsWith(":list") || p.EndsWith(":list_deleted")
                             || p.EndsWith(":read_usage") || p.EndsWith(":list_tokens") || p.EndsWith(":list_all")).ToArray();
}
