// <auto-generated />
// This file was generated by PulumiApiGenerator. Do not edit by hand.
#nullable enable

using System;
using System.Collections.Generic;
using System.Threading;
using System.Threading.Tasks;
using FastEndpoints;
using HappyPumi.Api.Contracts;
using HappyPumi.Api.State;

namespace HappyPumi.Api.Endpoints.Stacks;

/// <summary>
/// StartUpdateForRefresh
/// </summary>
public sealed class StartUpdateForRefreshEndpoint(UpdateLifecycle lifecycle) : Endpoint<StartUpdateForRefreshRequest, AppStartUpdateResponse>
{
    public override void Configure()
    {
        Post("/api/stacks/{orgName}/{projectName}/{stackName}/refresh/{updateID}");
        AllowAnonymous(); // TODO: replace with your auth policy (e.g. Roles(...), Policies(...))
        Description(b => b
            .WithTags("Stacks")
            .WithSummary("StartUpdateForRefresh")
            .WithDescription("Starts execution of a previously created refresh update. The update must have been created via CreateUpdateForRefresh first. Returns a lease token and version number for tracking the update. Returns 404 if the update does not exist, or 409 if another update is already in progress on the stack.")
            .WithName("StartUpdateForRefresh")
        );
    }

    public async override Task HandleAsync(StartUpdateForRefreshRequest req, CancellationToken ct)
    {
        var update = lifecycle.Start(req.UpdateId);
        if (update is null)
        {
            await Send.NotFoundAsync(ct);
            return;
        }

        // JournalVersion 0 => the CLI persists via full checkpoints, not journaling (see backend.go:2068).
        await Send.OkAsync(new AppStartUpdateResponse
        {
            Version = update.Version,
            Token = update.Token,
            JournalVersion = 0,
        }, ct);
    }
}
