...
/// CreateStack
/// </summary>
public sealed class CreateStackEndpoint(IStackStore stacks, IStackPermissionStore permissions) : Endpoint<CreateStackRequest, AppCreateStackResponse>
{
    ...
    public async override Task HandleAsync(CreateStackRequest req, CancellationToken ct)
    {
        ...
        // The creating user becomes the stack's admin collaborator (and its recorded creator), mirroring
        // Pulumi's behaviour where the creator owns the stack. Anonymous creators leave no grant.
        var creator = RequestActor.From(User)?.Login;
        if (!string.IsNullOrWhiteSpace(creator))
            permissions.SetUserPermission(stack.Coordinates, creator, StackPermissionLevel.Admin, isCreator: true);
